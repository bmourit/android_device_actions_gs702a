...
status_t OMXVce::loadedToIdle( )
{
    ...
    for(i = 0; i< mInputBufferCount; i++)
    {
		...
		CAMHAL_LOGDB("OMX_UseBuffer phyaddr (%p)\n", (void *)((private_handle_t *)((video_metadata_t *)mInputBuffers[i])->handle)->phys_addr);
		CAMHAL_LOGDB("OMX_UseBuffer virtaddr (%p)\n", (void *)((private_handle_t *)((video_metadata_t *)mInputBuffers[i])->handle)->base);
	}
...
status_t OMXVceFaceDetect::setFaceDetect(OMX_BOOL enable)
{
	...
	CAMHAL_LOGEB("mOmxHandle=%p!",mOmxHandle);
	CAMHAL_LOGEB("fdParam=%p!",&fdParam);
