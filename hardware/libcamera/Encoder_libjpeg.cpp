...
static void nv12ToYuyv(uint8_t* dst, uint8_t* y, uint8_t* uv, int width)
{
    if (!dst || !y || !uv)
    {
        return;
...
static void yu12ToYuyv(uint8_t* dst, uint8_t* y, uint8_t* u,uint8_t* v, int width)
{
    if (!dst || !y || !v ||!v)
    {
        return;
...
static void uyvyToYuyv(uint8_t* dst, uint32_t* src, int width)
{
    if (!dst || !src)
    {
        return;
...
static void resizeNV12(Encoder_libjpeg::params* params, uint8_t* src,  uint8_t* dst_buffer)
{
    ...
    in.y = src;
    in.u = src +  params->orig_width * params->orig_height;
    in.v = in.u + 1;

    out.y = dst_buffer;
    out.u = dst_buffer +  params->out_width * params->out_height;
    out.v = out.u + 1;
    ...
    {
        memcpy(dst_buffer, src, (resize.inputW*resize.inputH*12)>>3);
    }
    else
    {
        NVXXResize(&out, &in, &resize);
    }                           
}
...
static void resizeYU12(Encoder_libjpeg::params* params, uint8_t* src, uint8_t* dst_buffer)
{
    ...
    in.y = src;
    in.u = src +  params->orig_width * params->orig_height;
    in.v = in.u + (( params->orig_width * params->orig_height)>>2);

    out.y = dst_buffer;
    out.u = dst_buffer +  params->out_width * params->out_height;
    out.v = out.u + (( params->out_width * params->out_height)>>2);
...
size_t Encoder_libjpeg::encode(params* input)
{
    ...
    Rect bounds;
    bounds.left = left_crop;
    bounds.top = top_crop;
    bounds.right = in_width;
    bounds.bottom = in_height;

    mapper.lock((buffer_handle_t)input->src, CAMHAL_GRALLOC_USAGE, bounds, (void **)&src);
    src = (uint8_t *) ((unsigned int)src + input->src_offset);
    

    libjpeg_destination_mgr dest_mgr(input->dst, input->dst_size);

    // param check...
    CAMHAL_LOGDB("encode parameter:  \n\t"
                 "in_width: %d    \n\t"
                 "out_width:%d    \n\t"
                 "in_height: %d    \n\t"
                 "out_height:%d    \n\t"
    ...
    else if (input->format==CameraFrame::CAMERA_FRAME_FORMAT_RGB565)
    {
        bpp = 2;
        if ((in_width != out_width) || (in_height != out_height))
        {
    ...
    else if (input->format == CameraFrame::CAMERA_FRAME_FORMAT_YUV422I)
    {
        bpp = 2;
        if ((in_width != out_width) || (in_height != out_height))
        {
    ...
    row_u = src + out_width * out_height * bpp;
    row_v = row_u + ((out_width * out_height * bpp)>>2);
    while ((cinfo.next_scanline < cinfo.image_height) )
    {
        ...
        if (input->format==CameraFrame::CAMERA_FRAME_FORMAT_YUV420SP)
        {
            nv12ToYuyv(row_tmp, row_src, row_uv, out_width);
            row[0] = row_tmp;
        }
        else if (input->format==CameraFrame::CAMERA_FRAME_FORMAT_YUV420P)
        {
            yu12ToYuyv(row_tmp, row_src, row_u, row_v, out_width);
            row[0] = row_tmp;
        }
        else if(input->format==CameraFrame::CAMERA_FRAME_FORMAT_YUV422I)
        {
        ...
        else if(input->format==CameraFrame::CAMERA_FRAME_FORMAT_YUV420SP)
        {
            if (!(cinfo.next_scanline % 2))
            {
                row_u = row_u +  ((out_width * bpp)>>1);
                row_v = row_v +  ((out_width * bpp)>>1);
            }
        }
...
exit:
    mapper.unlock((buffer_handle_t)input->src);
