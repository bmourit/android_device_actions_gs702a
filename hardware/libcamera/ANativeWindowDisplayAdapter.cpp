...
void* ANativeWindowDisplayAdapter::allocateBuffer(int width, int height, const char* format, int &bytes, int numBufs)
{
    ...
    mGrallocHandleMap = new private_handle_t*[lnumBufs];
...
#else
              CameraFormatConverter::halFormatForCameraFormat(format)
#endif
			  );
    if (err != 0)
    {
    ...
    cameraformat = (CameraFrame::FrameFormat)toCameraFormat(format);
    ...
    for( i = 0;  i < mBufferCount-undequeued; i++ )
    {
        ...
        mapper.lock((buffer_handle_t) mGrallocHandleMap[i], CAMHAL_GRALLOC_USAGE, bounds, &vaddr);
        mVaddrMap[i] = vaddr;
...
uint32_t * ANativeWindowDisplayAdapter::getOffsets()
{
    ...
    if(mOffsetsMap == NULL)
    {
        ...
        {
            private_handle_t* handle =  (private_handle_t*) *(mBufferHandleMap[i]);
            mOffsetsMap[i] = 0;
        }
    }
...
bool ANativeWindowDisplayAdapter::handleFrameReturn()
{
    ...
    while (mapper.lock((buffer_handle_t) mGrallocHandleMap[i], CAMHAL_GRALLOC_USAGE, bounds, &vaddr) < 0)
    {
