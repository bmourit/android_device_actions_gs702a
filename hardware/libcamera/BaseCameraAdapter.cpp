...
void BaseCameraAdapter::CameraStreamBuffer::dumpBuffer()
{
	...
	int i;
    Mutex::Autolock lock(mLock);
	CAMHAL_LOGEB("======dumpBuffer=%p\n", mBuffer);
	CAMHAL_LOGEB("mRefCountSum=%d\n", mRefCountSum);
	for(i = 0; i < CameraFrame::MAX_FRAME_TYPE; i++)
	{
		CAMHAL_LOGEB("mRefCount[%d]=%d\n", i,mRefCount[i]);
	}
	CAMHAL_LOGEB("=======end dumpBuffer\n");
...
void BaseCameraAdapter::CameraStreamBuffer::returnFrame(CameraFrame::FrameType frameType)
{
	LOG_FUNCTION_NAME
    mStream->returnFrameCb(mBuffer, frameType);
	LOG_FUNCTION_NAME_EXIT
};
...
uint32_t BaseCameraAdapter::CameraStream::getRefCountByFrameTypes(uint32_t types)
{
	...
	ALOGE("CameraStream::getRefCountByFrameTypes =%d\n", sum);
	LOG_FUNCTION_NAME_EXIT
    return sum;
}
...
void BaseCameraAdapter::CameraStream::returnFrameCb(void *frameBuf,CameraFrame::FrameType frameType)
{
	LOG_FUNCTION_NAME
    mHub->returnFrameCb(frameBuf, frameType, mStreamType);
	LOG_FUNCTION_NAME_EXIT
}
...
void BaseCameraAdapter::CameraStreamHub::returnFrameCb(void *frameBuf, CameraFrame::FrameType frameType, CameraFrame::StreamType streamType)
{
	LOG_FUNCTION_NAME
    mBca->fillThisBuffer(frameBuf, frameType, streamType);
	LOG_FUNCTION_NAME_EXIT
};
...
void BaseCameraAdapter::returnFrame(void* frameBuf, CameraFrame::FrameType frameType, CameraFrame::StreamType streamType)
{
    ...
    if ( NO_ERROR == res)
    {
		...
		onReturnFrame(frameBuf, frameType, streamType);
        mStreamHub->returnFrame(frameBuf, frameType, streamType);
    }

}

void BaseCameraAdapter::onReturnFrame(void* frameBuf, CameraFrame::FrameType frameType, CameraFrame::StreamType streamType)
{
}
...
status_t BaseCameraAdapter::fillThisBuffer(void* frameBuf, CameraFrame::FrameType frameType, CameraFrame::StreamType streamType)
{
    status_t ret = NO_ERROR;
