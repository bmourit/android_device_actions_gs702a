...
int mkfs_vfat_main(int argc UNUSED_PARAM, char **argv)
{
 ...
 set_cluster_size:
	...
	if ((off_t)(volume_size_sect - act_reserve) < 4)    
		bb_error_msg_and_die("the image is too small for FAT32");
	sect_per_fat = 1;
	while (1) {
		while (1) {
			int spf_adj;
			uoff_t tcl = (volume_size_sect - act_reserve - NUM_FATS * sect_per_fat) / sect_per_clust;
 ...
 found_total_clust:
	...
	{ // boot and fsinfo sectors, and their copies
		...
		xwrite(dev, buf, bytes_per_sect * (act_reserve - backup_boot_sector));
	}
