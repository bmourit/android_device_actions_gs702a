...
/* $Id: nrx_lib.h 23882 2012-05-17 12:34:45Z peva $ */
...
typedef enum
{
   NRX_ENCR_DISABLED,   /**< Not encrypted */
   NRX_ENCR_WEP,        /**< WEP */
   NRX_ENCR_TKIP,       /**< TKIP */
   NRX_ENCR_CCMP,       /**< CCMP */
   NRX_ENCR_SMS4        /**< SMS4 */
...
typedef nrx_mac_addr_t m80211_mac_addr_t;

typedef struct
{
    m80211_mac_addr_t   peer;
    m80211_mac_addr_t   dev_addr;
    uint8_t             pri_dev_type[8];
    m80211_dev_name_t   dev_name;
...
typedef struct {
    ...
    m80211_mac_addr_t   peer_device_addr;
    m80211_mac_addr_t   peer_interface_addr;
    uint16_t            wps_method;
#define P2P_MAX_CHANNELS 50
    uint32_t            freq_list[P2P_MAX_CHANNELS];
    int                 persistent_group;
    uint8_t             peer_config_timeout;
} wpas_p2p_go_neg_res_t;


typedef struct {
    char    dev_name[32]; /* change to static allocation */
    uint8_t pri_dev_type[8];
    #define DRV_MAX_SEC_DEV_TYPES 5
    uint8_t sec_dev_type[DRV_MAX_SEC_DEV_TYPES][8];
...
typedef struct {
    int                 frame_type;
    int                 freq;
    m80211_mac_addr_t   src_addr;
    uint8_t             dialog_token;
    uint16_t            update_indicator;
    size_t              tlvs_len;
    //tlvs will follow
    uint8_t            *tlvs;
} wpas_p2p_serv_disc_t;
...
//Note: change also in wifi_engine.h

#define NANO_IW_NET_P2P   0x80
