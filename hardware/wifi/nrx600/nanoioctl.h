...
#define P2PSTART 40 /* if you add more NRX ioctl then inc the P2PStart */
...
#ifdef INCLUDE_P2P_SOFTAP
/* P2P */
/* struct for p2p and nrxapi */
struct nrx_ioc_p2p_set_params {
    struct nrx_ioc ioc;
    wpas_p2p_params_t value;
};

struct nrx_ioc_p2p_freq_params {
    struct nrx_ioc ioc;
    wpas_freq_params_t value;
};

struct nrx_ioc_p2p_generic_elem {
    struct nrx_ioc ioc;
    buf_t buffer;
};

struct nrx_ioc_p2p_set_ap_wps_ie {
    struct nrx_ioc ioc;
...
struct nrx_ioc_p2p_start_neg {
    struct nrx_ioc ioc;
    m80211_mac_addr_t peer_addr;
    int wps_method;
    int go_intent;
    m80211_mac_addr_t own_interface_addr;
...
struct nrx_ioc_p2p_prov_end {
    struct nrx_ioc ioc;
    int result;
    m80211_mac_addr_t peer_addr;
};

struct nrx_ioc_p2p_set_noa {
    struct nrx_ioc ioc;
    uint8_t  count;
    uint32_t start;
    uint32_t duration;
...
struct nrx_ioc_p2p_set_powersave {
    struct nrx_ioc ioc;
    int legacy_ps;
    int opp_ps;
    int ctwindow;
...
struct nrx_ioc_p2p_prov_disc_req{
    struct nrx_ioc ioc;
    m80211_mac_addr_t peer_addr;
    uint16_t config_methods;
};

struct nrx_ioc_p2p_sd_request{
    struct nrx_ioc      ioc;
    m80211_mac_addr_t   dst;
    buf_t               tlvs;
};

struct nrx_ioc_p2p_sd_response{
    struct nrx_ioc      ioc;
    int                 freq;
    m80211_mac_addr_t   dst;
...
struct nrx_ioc_p2p_reject{
    struct nrx_ioc      ioc;
    m80211_mac_addr_t   addr;
};

struct nrx_ioc_p2p_invite{
    struct nrx_ioc      ioc;
    m80211_mac_addr_t   peer;
    int                 role;
    m80211_mac_addr_t   bssid;
    buf_t               ssid;
    m80211_mac_addr_t   go_dev_addr;
    int                 persistent_group;
};

struct nrx_ioc_softap_wpa_ie {
   struct           nrx_ioc ioc;
   size_t           len;
   void *           buf;
};

struct nrx_ioc_softap_eapol {
    struct nrx_ioc      ioc;
    m80211_mac_addr_t   dst;
    uint16_t            proto;
    uint32_t            eapol_len;
...
struct nrx_ioc_softap_encryption {
    struct nrx_ioc      ioc;
    uint8_t             cipher;
    m80211_mac_addr_t   addr;
    int                 idx;
...
/* we continue from number 40 !!!! */
#define NRXIOCP2PSETFREQ          NRXIOW ('P', P2PSTART, p2p_freq_params)
#define NRXIOCP2PSETGENELEM       NRXIOW ('P', P2PSTART+1, p2p_generic_elem)
#define NRXIOCP2PSETAPWPSIE       NRXIOW ('P', P2PSTART+2, p2p_set_ap_wps_ie)
#define NRXIOCP2PFIND             NRXIOW ('P', P2PSTART+3, p2p_find)
#define NRXIOCP2PSTARTNEG         NRXIOW ('P', P2PSTART+4, p2p_start_neg)
#define NRXIOCP2PPROVEND          NRXIOW ('P', P2PSTART+5, p2p_prov_end)
#define NRXIOCP2PLISTEN           NRXIOW ('P', P2PSTART+6, p2p_find)
#define NRXIOCP2PSETPOWERSAVE     NRXIOW ('P', P2PSTART+7, p2p_set_powersave)
#define NRXIOCP2PINTRABSS         NRXIOW ('P', P2PSTART+8, uint8_t)
#define NRXIOCP2PPROVDISCREQ      NRXIOW ('P', P2PSTART+9, p2p_prov_disc_req)
#define NRXIOCP2PSDREQUEST        NRXIOW ('P', P2PSTART+10, p2p_sd_request)
#define NRXIOCP2PSDCANCELREQ      NRXIOW ('P', P2PSTART+11, uint64_t)
#define NRXIOCP2PSDRESPONSE       NRXIOW ('P', P2PSTART+12, p2p_sd_response)
#define NRXIOCP2PSERVICEUPDATE    NRXIO  ('P', P2PSTART+13)
#define NRXIOCP2PREJECT           NRXIOW ('P', P2PSTART+14, p2p_reject)
#define NRXIOCP2PINVITE           NRXIOW ('P', P2PSTART+15, p2p_invite)
#define NRXIOCP2PSETPARAMS        NRXIOW ('P', P2PSTART+16, p2p_set_params)
#define NRXIOCSOFTAPGETWPAIE      NRXIOWR('P', P2PSTART+17, softap_wpa_ie)
#define NRXIOCP2PSETNOA           NRXIOW ('P', P2PSTART+18, p2p_set_noa)
#define NRXIOCSOFTAPSENDEAPOL     NRXIOW ('P', P2PSTART+19, softap_eapol)
#define NRXIOCSOFTAPSETBCTYPE     NRXIOW ('P', P2PSTART+20, uint8_t)
#define NRXIOCSOFTAPSETENCRYPTION NRXIOW ('P', P2PSTART+21, softap_encryption)
#define NRXIOCSOFTAPCLIENTS       NRXIOR ('P', P2PSTART+22, uint32_t)
#endif /* INCLUDE_P2P_SOFTAP */
