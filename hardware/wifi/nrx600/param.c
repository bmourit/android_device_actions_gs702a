...
/* $Id: param.c 20260 2011-09-27 11:50:15Z geza $ */
...
/*!
  * @ingroup MISC
  * @brief <b>Get the number of assosiated clients, for softAP</b>
  *
  * Returns the number of associated stations, when running as softAP.
  * 
  * @param ctx NRX context that was created by the call to nrx_init_context().
  * 
  * @return 
  ...
  */
int 
nrx_get_softap_clients(nrx_context ctx, uint32_t *value)
{
   int ret;
   struct nrx_ioc_uint32_t param;
   NRX_ASSERT(ctx);
   ...
   ret = nrx_nrxioctl(ctx, NRXIOCSOFTAPCLIENTS, &param.ioc);
   if(ret)
      return ret;
