...
/* $Id: init.c 19623 2011-07-11 11:53:52Z peva $ */
...
int
nrx_ioctl_raw(nrx_context ctx, unsigned int cmd, void *data,int size)
{
    struct iwreq iwr;

    /* reset the wireless struct */
    memset(&iwr, 0, sizeof(iwr));

    /* set the interface name */
    strlcpy(iwr.ifr_name, ctx->ifname, IFNAMSIZ);

    /* pass the data like extra packet in wireless extension */
    iwr.u.data.length = size;
    iwr.u.data.pointer = data;

    /* send the wireless cmd */
 ...
 */
int
nrx_nrxioctl_raw(nrx_context ctx, unsigned int cmd, struct nrx_ioc *param,int size)
{
    param->magic = NRXIOCMAGIC;
    param->cmd = cmd;

    return nrx_ioctl_raw(ctx, SIOCNRXIOCTL, param, size);
}
